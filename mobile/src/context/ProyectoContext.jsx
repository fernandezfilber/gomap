...
export const ProyectosProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const protectedPaths = ['/dashboard', '/estadisticas'];
        const shouldFetch = protectedPaths.some(path => location.pathname.startsWith(path));
        if (!shouldFetch) return;

        fetchProyectos();
    }, [fetchProyectos, location.pathname]);
