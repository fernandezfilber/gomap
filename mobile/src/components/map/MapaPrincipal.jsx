...
const MapaPrincipal = ({ modo = 'select', setModo = () => { }, medirDistancia, setMedirDistancia }) => {
    ...
    const { tramos, crearTramo, eliminarTramo } = useTramos(proyectoSeleccionado?.id);
    ...
    const MapEvents = () => {
        ...
        mapRef.current = map;
        useEffect(() => {
            // Timer para long press táctil (reemplaza click derecho en móvil)
            let longPressTimer = null;
            let longPressLatlng = null;

            const abrirMenuCliente = (latlng) => {
                setModal({ show: true, type: 'cliente', data: { latitud: latlng.lat, longitud: latlng.lng } });
            };

            const handleClick = (e) => {
                // Click derecho en desktop → menú cliente
                if (e.originalEvent.button === 2) {
                    e.originalEvent.preventDefault();
                    abrirMenuCliente(e.latlng);
                    return;
                }
            ...
            // Touch start → iniciar long press timer (700ms)
            const handleTouchStart = (e) => {
                ...
                longPressTimer = setTimeout(() => {
                    if (longPressLatlng) abrirMenuCliente(longPressLatlng);
                }, 700);
            };
            ...
            map.getContainer().addEventListener('touchmove', handleTouchEnd);
            ...
            return () => {
                ...
                map.getContainer().removeEventListener('touchmove', handleTouchEnd);
            };
    ...
    return (
        <div className="relative h-full w-full bg-slate-950">
            {/* Stats Flotantes */}
            <div className="absolute top-4 left-4 lg:top-6 lg:left-6 z-[1001] bg-slate-950/80 backdrop-blur-xl border border-white/10 p-3 lg:p-6 shadow-[0_20px_50px_rgba(0,0,0,0.5)] text-white max-w-[130px] lg:max-w-xs rounded-[1.5rem] lg:rounded-[2rem]">
            ...
            {medirDistancia && puntosDistancia.length > 0 && (
                <div className="absolute top-4 right-4 lg:top-6 lg:right-6 z-[1002] bg-blue-600/10 backdrop-blur-xl border border-blue-500/30 px-6 py-5 rounded-[2rem] text-white shadow-2xl min-w-[220px]">
                    ...
                    <div className="flex items-baseline gap-1"><span className="text-5xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-br from-white to-slate-500">{puntosDistancia.length > 1 ? puntosDistancia.reduce((acc, p, i) => i === 0 ? 0 : acc + calcularDistancia(puntosDistancia[i-1][0], puntosDistancia[i-1][1], p[0], p[1]), 0) * 1000 : 0}</span><span className="text-lg font-bold text-blue-500">m</span></div>
            ...
            {/* Botón Mi Ubicación — en mobile queda encima del toolbox */}
            <button
                onClick={handleMiUbicacion}
                className="absolute right-4 z-[1001] bg-white p-3 rounded-full shadow-xl hover:bg-slate-50 transition-all text-blue-600 border border-slate-200"
                style={{
            ...
            <MapContainer center={mapCenter} zoom={18} maxZoom={22} className="h-full w-full" zoomControl={false}>
                ...
                <LayersControl position="bottomright">
                    ...
                    <LayersControl.Overlay checked name="Cables"><LayerGroup>{tramos.map(t => (<Polyline key={t.id} positions={t.path} pathOptions={{ color: t.colorVisual || '#ef4444', weight: 5, opacity: 0.9 }}><Popup><div className="text-center"><p className="font-bold mb-2">Cable ID: {t.id}</p><button onClick={() => eliminarTramo(t.id)} className="text-red-500">Eliminar</button></div></Popup></Polyline>))}</LayerGroup></LayersControl.Overlay>
                    ...
                    <LayersControl.Overlay checked name="Mufas"><LayerGroup>{mufas.filter(m => esCoordenadaValida(m.latitud, m.longitud)).map(m => (
                        <Marker key={m.id} position={[parseFloat(m.latitud), parseFloat(m.longitud)]} icon={iconoMufa} ref={el => el ? markerRefs.current.set(m.id, el) : markerRefs.current.delete(m.id)}>
                            <Popup><div className="text-center p-3"><p className="font-bold text-orange-400 text-xl mb-1">🌀 Mufa: {m.codigo}</p><p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider mb-2">Tipo: {m.tipo || 'N/A'}</p><div className="flex gap-3 mt-4"><button onClick={() => setModal({ show: true, type: 'mufa', data: m })} className="flex-1 bg-orange-600 text-white py-2 rounded-xl text-xs">Editar</button><button onClick={() => eliminarMufa(m.id)} className="flex-1 bg-red-600 text-white py-2 rounded-xl text-xs">Eliminar</button></div></div></Popup>
                        </Marker>
                    ))}</LayerGroup></LayersControl.Overlay>
                            ...
                            <Popup>
                                <div className="text-center p-3">
                                    ...
                                    <div className="flex gap-4 justify-center mt-4">
                                        <button onClick={() => setModal({ show: true, type: 'caja', data: c })} className="text-blue-500 hover:text-blue-400 font-medium">Editar</button>
                                        <button onClick={() => eliminarCaja(c.id)} className="text-red-500 hover:text-red-400 flex items-center gap-1 font-medium"><Trash2 size={16} /> Eliminar</button>
                                    </div>
                                </div>
                            </Popup>
                        </Marker>
            ...
            {puntosTemporales.length > 0 && (
                <div className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-slate-900 border border-slate-700 text-white px-8 py-5 shadow-2xl flex items-center gap-5 z-[1001] rounded-[2rem]">
                    ...
                    <button onClick={() => { 
                        ...
                        crearTramo({ path: puntosTemporales, proyectoId: proyectoSeleccionado.id, posteInicioId: nodoInicio.id, posteFinId: nodoFin.id, colorVisual: colorTramoTemporal })
                            .then(() => { setPuntosTemporales([]); setNodoInicio(null); setNodoFin(null); setModo('select'); })
                            .catch(err => alert(err.message));
                    }} className="bg-violet-600 hover:bg-violet-700 px-8 py-3 rounded-xl font-bold transition-all">Finalizar Tramo</button>
                    <button onClick={() => { setPuntosTemporales([]); setNodoInicio(null); setNodoFin(null); }} className="text-red-400 hover:text-red-300"><X size={28} /></button>
                </div>
            )}
            ...
            {modal.show && (
                <div className="fixed inset-0 bg-black/80 z-[1003] flex items-center justify-center p-4 backdrop-blur-sm">
                    ...
                    {modal.type === 'caja' && <FormCaja data={modal.data} mufas={mufas} onCancel={() => setModal({show:false})} onSuccess={() => { fetchCajas(); actualizarIconoPoste(modal.data?.posteId); }} crearCaja={crearCaja} actualizarCaja={actualizarCaja} />}
                    {modal.type === 'caja_rapida' && <FormCajaRapida coordenadas={{ latitud: modal.data.latitud, longitud: modal.data.longitud }} posteId={modal.data.posteId} onCancel={() => setModal({show:false})} />}
