    ...
    return (
        <>
            ...
            <aside className={`
                fixed
                top-0 left-0
                w-[85vw] max-w-[300px] lg:w-72
                bg-white h-screen flex flex-col text-slate-500 border-r border-slate-100 
                shadow-2xl z-[1002]
            ...
            `}>
                ...
                <div
                    className="border-b border-slate-50 flex items-center justify-between"
                    style={{
                ...
                >
                    <img src={logoFull} alt="GoMap" className="h-9 w-auto" />
                    <button 
                        onClick={onClose}
                        className="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-[#FF4500] bg-slate-50 hover:bg-red-50 rounded-full transition-colors border border-slate-100"
                ...
                <nav className="flex-1 p-6 space-y-8 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="space-y-2">
                        ...
                        <Link to="/estadisticas" className="block" onClick={() => { onClose(); }}>
                            <button className="flex items-center gap-3 w-full p-4 rounded-2xl hover:bg-slate-50 transition-all font-bold text-[10px] uppercase tracking-widest text-slate-500 hover:text-slate-900" style={{ minHeight: 'unset' }}>
                    ...
                    <div className="space-y-2 pt-6 border-t border-slate-50">
                        {JSON.parse(localStorage.getItem('user'))?.rol === 'ADMIN' && (
                            <Link to="/admin" className="block" onClick={() => { onClose(); }}>
                                <button className="flex items-center gap-3 w-full p-4 rounded-2xl bg-[#FF4500]/5 text-[#FF4500] hover:bg-[#FF4500] hover:text-white transition-all font-black text-[10px] uppercase tracking-widest mb-2 border border-[#FF4500]/10" style={{ minHeight: 'unset' }}>
                                    <ShieldAlert size={18}/> Panel Maestro
                                </button>
                            </Link>
                        )}
                        <button className="flex items-center gap-3 w-full p-4 rounded-2xl hover:bg-slate-50 transition-all font-bold text-[10px] uppercase tracking-widest text-slate-400 hover:text-slate-900" style={{ minHeight: 'unset' }}>
