...
const MapaPrincipal = ({ modo = 'select', setModo = () => { }, medirDistancia, setMedirDistancia }) => {
    ...
    const getPosteIcon = (posteId) => {
        ...
        return (tieneMufa || tieneCaja) ? getIconoPosteOcupado(zoomLevel) : getIconoPoste(zoomLevel);
    };
    ...
    const MapEvents = () => {
        ...
        useEffect(() => {
            ...
            return () => { map.off('click', handleClick); map.off('contextmenu', handleClick); map.off('zoomend', updateZoom); };
        ...
        <div className="relative h-full w-full bg-slate-950">
            ...
            <MapContainer center={mapCenter} zoom={18} maxZoom={22} className="h-full w-full" zoomControl={false} rotate={true} touchRotate={true} rotateControl={{ closeOnZeroBearing: false }}>
                ...
                <LayersControl position="bottomright">
                    <LayersControl.BaseLayer name="Google Satélite + Calles"><TileLayer url="https://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}" attribution='&copy; Google Maps' maxZoom={22} maxNativeZoom={20} /></LayersControl.BaseLayer>
                    <LayersControl.BaseLayer checked name="Google Street"><TileLayer url="https://mt1.google.com/vt/lyrs=m&x={x}&y={y}&z={z}" attribution='&copy; Google Maps' maxZoom={22} maxNativeZoom={20} /></LayersControl.BaseLayer>
                    <LayersControl.BaseLayer name="Sin nombres y detalles"><TileLayer url="https://{s}.basemaps.cartocdn.com/light_nolabels/{z}/{x}/{y}.png" attribution='' maxZoom={19} /></LayersControl.BaseLayer>
                    ...
                    <LayersControl.Overlay checked name="Clientes">
                        <LayerGroup>
                            {clientes.filter(cl => esCoordenadaValida(cl.latitud, cl.longitud)).map(cl => {
                                ...
                                return (
                                    <Fragment key={cl.id}>
                                        <Marker position={[parseFloat(cl.latitud), parseFloat(cl.longitud)]} icon={getIconoCliente(zoomLevel)} ref={el => el ? markerRefs.current.set(cl.id, el) : markerRefs.current.delete(cl.id)}>
                                            <Popup>
