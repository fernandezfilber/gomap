        ...
        <div className="relative h-full w-full bg-slate-950">
            ...
            <MapContainer center={mapCenter} zoom={18} maxZoom={22} className="h-full w-full" zoomControl={false}>
                ...
                <LayersControl position="bottomright">
                    ...
                    <LayersControl.Overlay checked name="Cables"><LayerGroup>{tramos.map(t => (<Polyline key={t.id} positions={t.path} pathOptions={{ color: t.colorVisual || '#ef4444', weight: 5, opacity: 0.9, offset: getOffset(t) }}><Popup><div className="text-center"><p className="font-bold mb-2">Cable ID: {t.id}</p><p className="text-xs text-slate-500 mb-2">{t.capacidadHilos || 48} Hilos Totales</p><div className="flex flex-col gap-2"><button onClick={() => { setTramoBaseParaContinuar(t); setPuntosTemporales([t.path[t.path.length - 1]]); setNodoInicio({ id: t.posteFinId || t.posteInicioId, tipo: 'poste' }); setModo('tramo'); }} className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-1 px-3 rounded text-xs">Continuar Tramo</button><button onClick={() => eliminarTramo(t.id)} className="text-red-500 hover:text-red-400 font-bold text-xs border border-red-500 rounded py-1">Eliminar</button></div></div></Popup></Polyline>))}</LayerGroup></LayersControl.Overlay>
